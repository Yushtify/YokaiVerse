    ...
    return entries.filter((entry) => {
        ...
        const hasPopularTag =
            ...
            Object.values(tags).some(
                (v) =>
                    typeof v === "string" && v.toLowerCase() === "popular",
            );
...
function setupListerUI(
    ...
    const clone = listerTemplate.content.cloneNode(
        true,
    ) as DocumentFragment;
...
function populateCard(clone: DocumentFragment, entry: AnimeEntry): void {
    ...
    if (authorEl)
        authorEl.textContent = data.author || CONFIG.defaults.author;
    ...
    for (const chunk of chunks) {
        const settled = await Promise.allSettled(
            chunk.map(async ([id, root]) => {
                const data = await fetchJSON<AnimeInfo>(
                    `${root}/information.json`,
                );

...
/**
 * animeInfHandler.ts
 * Info Page - Debug Enabled Modular Renderer (Anime)
...
declare global {
  interface Window {
    findAnime: (id: string, unitIndex: number, subUnitIndex: number) => void;
  }
}
...
const CONFIG = {
  ...
  // Global window function invoked when a sub-unit (episode) card is clicked.
  onSubUnitClick: (id: string, unitIndex: number, subUnitIndex: number) => {
    window.findAnime(id, unitIndex, subUnitIndex);
  },
} as const;
...
async function LoadModularInfo(seriesID: string): Promise<void> {
  try {
    ...
    await listSeries({
      root: seriesRootSrc,
      // This file only ever runs on /info/anime, so the mode is already known —

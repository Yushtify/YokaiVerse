...
function saveData() {
  // This is where will be saving the data perm to the DB where they left off
}
...
function updateCurrentTime(e: HTMLInputElement, value: string) {
...
function LoadVideoData(
  ...
  // say it so we know what we have sent as null or anything but string
  if (!TitleData || !DescData || !UploaderData || !ViewData || !UploadDateData)
    return console.error(
      "One of the following is nil:",
      TitleData,
      DescData,
      UploaderData,
      ViewData,
  ...
  // everything should be valid so find the comps and assign.
  const videoTitleElement = document.getElementById(
    "videoTitle",
  ) as HTMLParagraphElement; // video title
  const pageTitleElement = document.getElementById(
    "pageVideoTitle",
  ) as HTMLParagraphElement; // page vid title
  const pageDescElement = document.getElementById(
    "pageVideoDesc",
  ) as HTMLParagraphElement; // page vid desc
  const videoUploaderElement = document.getElementById(
    "videoUploader",
  ) as HTMLParagraphElement; // video uploader
  const pageUploaderElement = document.getElementById(
    "pageVideoUploader",
  ) as HTMLParagraphElement; // page vid uploader
  const pageUploaderImgElement = document.getElementById(
    "pageVideoUploaderIMG",
  ) as HTMLImageElement; // page vid uploader
  const pageViewsElement = document.getElementById(
    "pageVideoViews",
  ) as HTMLParagraphElement; // page vid views
  const pageUplodaDateElement = document.getElementById(
    "pageVideoUpload",
  ) as HTMLParagraphElement; // page vid views

  if (!videoTitleElement || !pageTitleElement)
    return console.error("[LoadVideoData] one of the DOM elements nil");
  else {
    // they are valid
    videoTitleElement.innerHTML = TitleData;
    pageTitleElement.innerHTML = TitleData;
    pageDescElement.innerHTML = DescData;
    videoUploaderElement.innerHTML = UploaderData;
    pageUploaderElement.innerHTML = UploaderData;
    pageUploaderImgElement.src = uploaderIMG;
    pageViewsElement.innerHTML = ViewData + " Views";
    pageUplodaDateElement.innerHTML = UploadDateData;
  }
}

async function enableCustomVideoPlayer(videoRootSource: string) {
  // HAVE TO EXIST ITS CRITICAL
  if (!videoRootSource) return console.log(videoRootSource);
  else console.log("The Video Root is:", videoRootSource);
  let videoSource = videoRootSource + "/video";
  const posterSource = videoRootSource + "/cover.webp";
  const uploaderIMG = videoRootSource + "/uploader.webp";
  const videoInformationJSON = videoRootSource + "/information.json";

  // Thee needs to exist or will not work
  if (!posterSource || !videoInformationJSON) {
    return console.error("Source paths are invalid!");
  }
  const VideoPlayer_Container = document.getElementById(
  ...
  if (!videoPlayer || !VideoPlayer_Container)
    return console.error(
      "enableCustomVideoPlayer: Required DOM elements not found!",
      videoPlayer,
      videoPlayer_Container,
    );

  // Get the information form JSON
  try {
    const response = await fetch(videoInformationJSON);
    if (!response.ok) throw new Error("JSON file not found");
    const data = await response.json();

    // Read JSON and assing things.
    const TitleData = data.title || "Untitled Video"; // Title of the video
    const DescData = data.desc || "This Video has no descriptions."; // Description of the video
    const UploaderData = data.uploader || "Unknown Uploader";
    const ViewData = data.views || 0; // Views of the video
    const UploadDateData = data.uploadDate || "Unknown Date"; // Upload date of the video
    LoadVideoData(
      TitleData,
      DescData,
      UploaderData,
    ...
    console.error("Error loading video information:", error);
    videoSource = "";
  }

  VideoPlayer_Container.classList.remove("hidden");
  VideoPlayer_Container.classList.add("flex");
  ...
  videoPlayer
    .play()
    .catch((e) => console.warn("Autoplay blocked or failed:", e));

  addListeners();
}

// This code loads when the page is /watch
document.addEventListener("DOMContentLoaded", async () => {
  const params = new URLSearchParams(window.location.search);
  const videoID = params.get("v"); // URL'deki ID'yi yakalar

  if (!videoID) return; // Stop if not ID

  const videoID_JSON = "/videoIds.json";

  try {
    const response = await fetch(videoID_JSON);
    const data = await response.json();
    const videoRootSRC = data[videoID];

    if (videoRootSRC) {
      // The page is now /watch shouldn't be giving errors
      enableCustomVideoPlayer(videoRootSRC);
    }
  } catch (error) {
...
function findVideo(videoID: string) {
  // Opens the page with the id (Example: /watch?v=badApple)
  window.location.href = `/watch?v=${videoID}`;
}
(window as any).findVideo = findVideo;

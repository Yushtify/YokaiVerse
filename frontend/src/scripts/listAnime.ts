...
const DEACTIVE_CLASS =
  "relative flex flex-row items-center justify-start min-w-fit w-fit h-fit gap-2 rounded-3xl p-3 hover:bg-LB_Secondary hover:dark:bg-DB_Secondary text-nowrap font-semibold text-left text-[min(6vw,0.9rem)] text-LT_Secondary dark:text-DT_Secondary transition-all duration-300 ease-in-out overflow-hidden cursor-pointer capitalize";
...
export async function listAnime(options: ListAnimeOptions): Promise<void> {
  ...
  if (!mainWrapper || !seasonTemplate || !cardTemplate) {
    console.error(
      "[listAnime]: Required elements not found in DOM." +
        mainWrapper +
        seasonTemplate +
        cardTemplate,
    );
  ...
  for (let si = 0; si < seasons.length; si++) {
    ...
    for (const { index: epIndex, data: epData } of episodes) {
      ...
      if (cardBtn) {
        ...
        if (dur) {
          dur.innerHTML =
            '<span translate="no" class="material-symbols-rounded">schedule</span> —:——';
          ...
          if (videoURL) {
            ...
            getVideoDurationAsync(videoURL, (duration) => {
              ...
              if (pendingDurations === 0)
                updateMetadata(totalSeconds, globalEpCount);
            });
          }
        }
...
function updateMetadata(totalSeconds: number, count: number): void {
